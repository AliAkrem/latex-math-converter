...
import { addSpacing } from "../converters";
...
describe("Latex Converter", () => {
  ...
  test("with spacing option", () => {
    ...
    expect(converter.convert("2+3", { addSpacing: true })).toBe("2 + 3");
    expect(converter.convert("2-3", { addSpacing: true })).toBe("2 - 3");
    expect(converter.convert("2=2", { addSpacing: true })).toBe("2 = 2");
    expect(converter.convert("2<3", { addSpacing: true })).toBe("2 < 3");
    expect(converter.convert("2>3", { addSpacing: true })).toBe("2 > 3");
  });
  ...
  test("error handling", () => {
    ...
    expect(() => converter.addSymbol("newSymbol", "⊕")).toThrow(
      "LaTeX command must start with \\"
    );
  });
